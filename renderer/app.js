...
async function sendCommand(overrideText = null, isActivation = false) {
  ...
  if (currentAudio) {
    ...
    if (!isActivation) {
      // V5.6: Rapid multi-command workflow — keep input active
      if (result.action && result.action.startsWith('execute:')) {
          setUIState(APP_STATE.LISTENING);
      } else {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log("V5 Initialized. Monitoring for:", wakeWords);
